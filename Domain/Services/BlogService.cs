...
namespace Domain.Services
{
    public class BlogService:IDisposable
    {
        ...
        {
            return _context.Blogs.OrderBy(b => b.Id).ToPagedList(pageNumber, pageSize);
        }
        ...
        {
            return _context.Blogs.FirstOrDefault(x => x.UserId == userId && x.Status == BlogStatus.Draft);
        }

        public Blog FindWithLinkReview(int id)
        {
            return
                _context.Blogs.Where(b => b.Status == BlogStatus.Publish && b.Id == id)
                    .Include(b => b.Links)
                    .Include(b => b.Reviews)
                    .FirstOrDefault();
        }
        ...
        {
            return _context.Blogs.Include(b=>b.Links).FirstOrDefault(x => x.Id == id && x.UserId == userId);
        }
        ...
        private void FixBlogLink(Blog blog, string[] links, int[] linkIds, string[] linkDescriptions)
        {
            ...
            for (int i = 0; i < linkIds.Length; i++)
            {
                ...
                if (!link.Url.Equals(links[i].Trim(), StringComparison.OrdinalIgnoreCase)||!link.Description.Equals(linkDescriptions[i].Trim(),StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    link.Invalide = false;
                    _context.LinkInvalids.RemoveRange(link.LinkInvalids.ToList());
